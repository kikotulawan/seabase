    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <input type="hidden" name="salary_scale_id" id="salary_scale_id" value="{{ $salary_scale->id }}">
          <input type="hidden" name="principal_id" id="principal_id" value="{{ $salary_scale->principal_id }}">
...
<script>
    ...
    function details(){
      ...
      details = $('#details').DataTable({
                  ...
                  lengthChange:false,
                  autoWidth:true,
                  ajax: "{{ route('salaryscaledetails',$salary_scale->id) }}",
                  columns: [
              ...
              $('form[name=form]').submit(function(e){
                  ...
                  if($('form[name=form] #id').val()===""){
                      url="{{ route('salaryscaledetails.store') }}";
                  }else{
                      url="{{ route('salaryscaledetails.update') }}";
                  }
                  ...
                  error:function(err)
                      {
                          if(err.status===422){
                              ...
                              $('form[name=form] #' +key).addClass('is-invalid');
                              $('form[name=form] #' +key).focus();
                              $('form[name=form] #'+key+"_help").text(value[0]);
                              })
                          }
                      }
                  })
              ...
              $('#details').on('click', '.edit', function () {
                  ...
                  $.get("{{ route('salaryscaledetails.index') }}" +'/' + id +'/edit', function (data) {
                      clearForm('form[name=form]');
                      $('#details_modal .modal-title').html('Edit');
                      $('#details_modal').modal('show');

                      $('form[name=form] #id').val(data.id);
                      $('form[name=form] #description').val(data.description);
                      $('form[name=form] #monthly').val(data.monthly);
                      $('form[name=form] #daily').val(data.daily);
                      $('form[name=form] #percentage').val(data.percentage);
                      $('form[name=form] #days').val(data.days);
                      $('form[name=form] #remarks').val(data.remarks);
                      $('form[name=form] #rank_id').val(data.rank_id).attr('selected','selected').trigger('change');

                      if(data.add_to_total==1){
                        $('form[name=form] #add_to_total').attr('checked','checked')
                      }
              ...
              $('#details').on('click', '.delete', function () {
                  ...
                  if($confirm == true ){
                      $.ajax({
                          type: "DELETE",
                          url: "{{ route('salaryscaledetails.store') }}"+'/'+id,
                          success: function (data) {
                              details.ajax.reload();
