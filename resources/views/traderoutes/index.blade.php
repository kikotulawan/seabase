      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Trade Routes</h3>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            @include('traderoutes.form')
...
<script type="text/javascript">
  $(function () {
    var table = $('.data-table').DataTable({
        ...
        lengthChange:false,
        pageLength: 5,
        ajax: "{{ route('allTradeRoutes') }}",
        columns: [
            {data: 'route_name', name: 'route_name'},
            {data: 'options', name: 'options', orderable: false, searchable: false}
        ],
        @can('trading-route-create')
        dom: "lBtipr",
            buttons: {
            buttons: [
                {
    ...
    $('#form').submit(function (e) {
        ...
        var url;
        if($('#id').val()===""){
          url="{{ route('traderoutes.store') }}";
        }else{
          url="{{ route('traderoutes.update') }}";
        }
        $.ajax({
          data: $('#form').serialize(),
          url: url,
    ...
    $('.data-table').on('click', '.edit', function () {
      var id = $(this).data("id");
      $.get("{{ route('traderoutes.index') }}" +'/' + id +'/edit', function (data) {
          $('form[name=form]')[0].reset();
          $('#form').find('input,small').removeClass('is-invalid').text('');
          $('#modal .modal-title').html('Edit Trade Route');
          $('#modal').modal('show');
          $('#id').val(data.id);
    ...
    $('.data-table').on('click', '.delete', function () {
      ...
      if($confirm == true ){
          $.ajax({
              type: "DELETE",
              url: "{{ route('traderoutes.store') }}"+'/'+id,
              success: function (data) {
                  table.draw();
